...
import { useListFolders } from '@/api/endpoints/folders/folders'
...
export default function AppsIndex() {
  ...
  if (folderFilter) queryParams.folder_id = folderFilter

  const { data: apps, isError, refetch, isFetching, isPending } = useListApps(queryParams, {
    query: { placeholderData: keepPreviousData },
  })

  // FolderSidebar also calls useListFolders(); React Query dedupes by key,
  // so this is a free read used to feed the per-card "Move to folder" menu.
  ...
  return (
    <div className="flex h-full flex-1 flex-col gap-6 p-4 md:p-6">
      ...
      <div className="flex flex-1 flex-row gap-6">
        <FolderSidebar activeFolderId={folderFilter} onSelect={setFolderFilter} />
        ...
        <div className="min-w-0 flex-1">
          {isPending || apps === undefined ? (
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {apps.map((app) => (
                <AppCard key={app.id} app={app} folders={folders} />
              ))}
            </div>
          ) : (
