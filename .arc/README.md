...
```
appstorecat/
├── server/           # Laravel 13 API (gateway + DB owner)
├── web/          # React 19 SPA
├── scraper-ios/  # App Store scraper (Fastify + TypeScript)
├── scraper-android/     # Google Play scraper (FastAPI + Python)
...
Foundation patterns that all server code must follow.
...
Standards, testing, git workflow, web, and tooling.
...
- **web.md** — React 19, TypeScript, TanStack Query, page structure
...
**When building server endpoints:**
...
9. **06-conventions/web.md** — React + TanStack Query

**For services/connectors:**
10. **04-services/connectors.md** — HTTP connector pattern (server → scrapers)
...
docker compose exec server php ./vendor/bin/pint
docker compose exec server php artisan test

# Run scraper tests
docker compose exec scraper-android pytest
docker compose exec scraper-ios npm test
